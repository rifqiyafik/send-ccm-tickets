...
function formatTelegramGroupHelp() {
  return [
    ...
    "",
    "ℹ️ *Ketik `/help` di private chat untuk command admin.*",
  ].join("\n");
}
...
function formatTelegramPersonalHelp() {
  return [
    ...
    "- `/status` : Kondisi koneksi & sesi aktif",
    "- `/sessions` : Daftar sesi tersimpan",
    "- `/login [id]` : Login / koneksikan sesi WhatsApp",
    "- `/stop [id]` : Putus koneksi sementara",
    "- `/logout [id]` : Unlink linked device WhatsApp",
    "- `/delete_session <id>` : Hapus credential sesi",
    "",
    "### 📤 Mode Caption Excel",
    "- `.import` : Normal ke WhatsApp (tiket + Excel + summary)",
    "- `.update` : Detail eskalasi saja ke WhatsApp",
    "- `.summary` : Rekap report saja ke Telegram",
    "- `.reminder` : Reminder tiket unresolved",
    "- `.special` : Kirim ulang tiket (bypass sent_tickets)",
    "- `... manual` : Kirim via Telegram target tanpa WhatsApp (misal: `.update manual`)",
    "",
    "### ⚙️ Pengaturan & Akses",
    "- `/register [id]` : Whitelist chat Telegram",
    "- `/whitelist` : Daftar whitelist Telegram",
    "- `/env` / `/change_env` : Ganti environment config",
  ].join("\n");
}
...
function formatStartMessage() {
  return [
    ...
    "",
    "🤖 Bot otomatisasi eskalasi tiket Customer Complaint Management (CCM) Telkomsel Region Sumbagut.",
    "",
    "🚀 **Fitur Utama Bot:**",
